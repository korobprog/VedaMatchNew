...
- [ ] Применить миграцию `union_skeleton` к рабочей базе и перезапустить seed.
- [ ] Фаза B — запросы и приватность: эндпоинты над `UnionConnectionRequest` (отправить/принять/отклонить), открытие контактов после взаимного согласия, применение настроек приватности в выдаче.
