...
- показывать только проверенным пользователям.

Закрытые сервисы открываются только для **проверенных пользователей**.
...
4. Текст под кнопкой:
   - “Вы можете продолжить пользоваться порталом уже сейчас. Закрытые сервисы для проверенных пользователей откроются после проверки.”
...
- [x] Применить миграцию `union_chat` к рабочей базе данных.
- [x] Ручное тестирование MVP-сценария с двумя пользователями:
  - пользователь A и пользователь B заполняют Union-профили;
  - A видит B в рекомендациях с корректной совместимостью и фильтрами;
  - A отправляет заявку, B принимает;
  - после взаимного согласия открываются разрешенные контакты;
  - пользователи переходят в чат и обмениваются сообщениями.

#### Следующий roadmap Union после MVP-проверки

1. **Страница связей и заявок**
   - [ ] Добавить страницу `/union/connections` для входящих, исходящих и принятых связей.
   - [ ] Дать пользователю быстрые действия: принять, отклонить, открыть профиль, перейти в чат.

2. **Уведомления и возвращаемость**
   - [ ] Показать индикаторы новых заявок и сообщений в Union-навигации.
   - [ ] Подготовить backend-основу для будущих email/Telegram/push-уведомлений.

3. **Улучшение качества рекомендаций**
   - [ ] Сделать объяснение совместимости более понятным: сильные совпадения, слабые места, недостающие данные.
   - [ ] Добавить анти-факторы или предупреждения, если профили явно не подходят друг другу.
   - [ ] Рассмотреть пресеты весов намерений: семья, дружба, служение, проект/бизнес.

4. **Безопасность и доверие**
   - [ ] Добавить блокировку пользователя.
   - [ ] Добавить жалобу на профиль или сообщение.
   - [ ] Добавить настройку “не показывать мой профиль в рекомендациях”.
   - [ ] Подготовить модерационный статус профиля для будущей ручной проверки.

5. **Социальные функции**
   - [ ] Добавить избранные профили.
   - [ ] Добавить личную заметку к профилю/связи.
   - [ ] Разделить рекомендации по сценариям: семья, дружба, служение, проект.

**Рекомендуемый порядок:** сначала закрыть миграцию и ручной MVP-тест, затем реализовать страницу связей и заявок как следующий ключевой UX-шаг.
