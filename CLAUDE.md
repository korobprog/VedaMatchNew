...
мобильного приложения (например, сервис открывается в приложении только
внешней ссылкой и в вебе не появилось нового экрана, который стоило бы
отразить). По умолчанию (строки нет) воркфлоу `.github/workflows/web-app-parity.yml`
заводит карточку «ДОГНАТЬ. <Сервис>: …» в колонке VedaMath-Native доски
«Работа» — молчание не освобождает от догона. Агент/разработчик приложения
разбирает такие карточки как входящую очередь.
