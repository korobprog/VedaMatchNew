...
pnpm --filter @vedamatch/web test          # vitest, jsdom
...
- Эталон соблюдения — **Market** и **Chat**. Union нарушает контракт
  исторически: `union-profile.service.ts` и `union-connection.service.ts`
  инжектят `UsersService`; копировать это устройство нельзя. Переписка портала
...
PR, трогающий `apps/web/src/app/<service>/`, `apps/web/src/components/<service>/`
или `apps/api/src/modules/<service>/` сервиса каталога (белый список
`CATALOG_SERVICES` в `scripts/web-app-parity/changed-paths-to-services.mjs`),
обязан содержать в описании строку `Затрагивает приложение: да` или
`Затрагивает приложение: нет`. «Нет» — только когда изменение недостижимо из
приложения. Без строки воркфлоу `.github/workflows/web-app-parity.yml` заводит
карточку «ДОГНАТЬ. <Сервис>: …» в колонке VedaMath-Native доски «Работа».
Папки вне белого списка (`admin/`, `notifications/`, `landing/` и прочие
портальные) карточку не порождают никогда.
...
Воркеров у сервисов много (`*-worker.service.ts`), образец для новой фоновой
стадии — `MotivationWorkerService`: тик раз в 30 с под Redis-лизом
(`SET NX PX`), клейм задачи через `updateMany` с проверкой статуса, ретраи и
восстановление зависших по `updatedAt`.
...
- **Контраст ≥ 4.5:1** (3:1 для ≥24px либо ≥18.66px жирного), считать поверх
  ...
  `background-color`. `--vm-gold` мелким текстом на светлой теме ниже порога —
  не брать. На подкрашенной карточке запас съедается даже у своего акцента,
  поэтому мерить каждый раз на месте; замеры — в комментариях у токенов в
  `globals.css`.
