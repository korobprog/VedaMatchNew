...
- **Контраст ≥ 4.5:1** (3:1 для ≥24px либо ≥18.66px жирного), считать поверх
  ...
  тёмная: `--vm-magenta` 4,62:1 / 6,13:1 — годится, `--vm-cyan` 4,53:1 /
  ...
  комментарий у токенов в `globals.css`); старые цифры 4.46:1 и 3.75:1
  относятся к прежним значениям и в коде уже не встречаются. Цифры выше — для
  `--vm-bg-0`; на подкрашенной карточке запас съедается, и акцент проваливается
  даже свой: на `bg-magenta/5` та же магента даёт 4,26:1, а золото 3,37:1.
