...
Статус: реализовано 2026-07-29, проверено на локальном стенде. Сервис включён в каталоге со `status: active` по решению владельца продукта — раньше, чем предполагала эта спека, поэтому на карточках пока нет превью, а голосов и закладок нет вовсе (фаза B).
