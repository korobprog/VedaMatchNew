...
remainingBatchBytes, onEntry): Promise<ExpandedArchive>` — сколько записей
взято и почему разбор кончился раньше архива. Сами записи уходят в `onEntry`
**по одной, сразу после заливки**: позицию по каждой заводит
`MusicIngestProcessService` (доставка в Prisma не ходит вовсе, так же устроен
`fetchUrl`), и ключ попадает в базу до того, как что-нибудь успеет упасть.
Списком в конце их собирать нельзя: одной транзакцией на две сотни строк
терялся весь комплект объектов разом, а следующая попытка распаковывала архив
заново с новыми ключами. Порядок требует номера дорожки, а он известен только
...
Между `dns.lookup` и соединением остаётся окно DNS rebinding: имя,
проверенное как публичное, к моменту запроса может резолвиться в петлю.
Закрыть его можно только соединением по уже проверенному адресу с подменой
`Host`, чего `fetch` не даёт — понадобится свой `undici.Agent` с
`connect.lookup`. Ручка доступна одним админам, поэтому в этой работе окно
оставлено и записано комментарием в шапке `music-ingest-fetch.service.ts`.
