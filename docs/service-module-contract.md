...
> Оговорка про эталон: `union-chat.service.ts` инжектит `UsersService` — это нарушение правила ниже, оставшееся с ранних этапов. Копировать его нельзя; свежий пример соблюдения контракта — **Market** (`apps/api/src/modules/market/`), где данные профиля читаются read-only через `PrismaService`, а общие хелперы (`is-admin.ts`, транслитерация слага, работа с S3) продублированы внутри модуля вместо импорта из чужого.
...
- Наравне с `User` разрешено read-only читать `UserBlock` — это портальная инфраструктура модерации, а не сервисная модель: блокировка пользователя обязана действовать во всех сервисах сразу, иначе заблокированный продолжит попадаться в выдаче. Список исчерпывающий: расширять его новыми таблицами без правки этого пункта нельзя, «оно же почти портальное» — не аргумент.
