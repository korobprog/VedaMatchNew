...
`singleFlight` целиком и подписан на `tokenAuthority.subscribe(...)`: если
токены обновились или пропали не по его собственной инициативе (тот же
фоновый путь), UI узнаёт об этом сразу, а не на следующем своём таймере.
