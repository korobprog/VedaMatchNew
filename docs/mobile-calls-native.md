...
обновления/перерыва»). Теперь токены стираются, только когда сервер явно
ответил `401`/`403` — любой другой статус (включая отсутствующий) оставляет
пару как есть и отдаёт текущий access, чтобы повторная попытка случилась
позже. Тесты на оба случая (`token-authority.spec.ts`): явный 401/403
стирает, 5xx/неопознанная ошибка — нет.
