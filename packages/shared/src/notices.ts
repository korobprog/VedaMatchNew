...
export interface NoticeDto {
  ...
  isMine: boolean;
  /**
   * Смотрящему можно удалить объявление: автору — своё, администратору
   * Объявлений — любое (VED-42). Отдельно от `isMine`: админ не автор, и
   * остальные кнопки автора ему не нужны.
   */
  canDelete: boolean;
}
