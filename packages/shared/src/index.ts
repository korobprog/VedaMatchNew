...
export interface AccessTokenPayload {
  ...
  apiScopes?: string[];
  /**
   * Человек, чьим ключом действует ИИ-агент. Есть только у агентского ключа:
   * действующее лицо — служебный аккаунт, но отвечает за сделанное тот, кто
   * ключ выпустил, и история «Работы» показывает обоих.
   */
  onBehalfOf?: string;
}
