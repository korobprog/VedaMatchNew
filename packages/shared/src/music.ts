 ...
 */
export type MusicTrackSort = 'fresh' | 'popular' | 'title';
 ...
 * алфавиту запись лежит там же, где лежала вчера.
 *
 * С VED-165 это ещё и единственный порядок, который видит человек на витрине:
 * ряд «Порядок» из панели «Фильтры» убран целиком (в панели остались только
 * «Стиль» и «Исполнитель»), и веб параметра `sort` больше не ставит. Сам
 * параметр API принимает по-прежнему — старые ссылки и другие клиенты не
 * ломаются.
 */
