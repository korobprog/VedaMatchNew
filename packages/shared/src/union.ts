...
export interface UnionPhoto {
  ...
  url: string;
  /**
   * Уменьшенная копия того же снимка. Плитке, миниатюре и превью в стопке
   * полноразмерный снимок не нужен: браузер распаковывает его целиком, и на
   * телефоне десяток таких распаковок съедает всю память вкладки.
   *
   * `null` — у снимков, залитых до появления копий, и пока по ним не прошёл
   * `gallery:shrink`. Читающая сторона обязана падать обратно на `url`.
   */
