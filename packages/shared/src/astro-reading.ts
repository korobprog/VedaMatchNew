...
export interface AstroQuotaState {
  ...
  readingsLeft: number;
  /**
   * Дневной потолок разборов. `0` — без лимита: на бете счёт не ведётся, и
   * расход сдерживает только общий бюджет токенов. Отдельный флаг заводить не
   * стали — «ноль в сутки» не значит ничего осмысленного, и путать его не с чем.
   */
