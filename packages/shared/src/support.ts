...
export const PLAN_FEATURES: string[] = [
  'Все сервисы платформы в одном аккаунте',
  '«Знакомства в Благости»: анкеты, рекомендации, чаты',
  '«Астрология»: ведическая карта рождения и совместимость',
  '«Библиотека» и «Образование»: чтение офлайн и без ограничений',
  '«Вдохновение», «Общение» и «Рынок» — практика, община и объявления',
...
export interface DonationRequisite {
  ...
  value: string;
  /**
   * Пояснение под значением: банки, которые принимают перевод по этому
   * номеру («Сбербанк, ВТБ, Озон-банк»), сеть у крипты, валюта. Нужно, чтобы
   * человек заранее видел, куда уйдут деньги по номеру телефона (VED-12).
   * Пусто или нет поля — строка показывается как раньше, без второй строки.
   */
  note?: string;
}
