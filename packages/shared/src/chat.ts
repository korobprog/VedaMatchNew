...
/**
 * `sourceService` вложения `call`, которым сервер отмечает начало группового
 * звонка в ленте беседы: «Звонок начался · [Войти в звонок]». `sourceId` —
 * id комнаты, по нему клиент сверяет карточку с идущим звонком и ведёт в
 * тот же вход, что и плашка. Когда комната закрывается, сервер правит ту же
 * карточку (`message.updated`): `durationSec` заполняется, `subtitle`
 * становится длительностью. Заполненный `durationSec` и есть признак
 * «звонок завершён» — отдельного поля статуса у вложения нет.
 *
 * Запись о звонке один на один остаётся с `sourceService: 'chat'`.
 */
export const CHAT_GROUP_CALL_MESSAGE_SOURCE = 'chat-group-call';
