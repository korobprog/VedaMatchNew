...
export interface MotivationPostDto {
   ...
   *
   * `id` — чтобы подпись под рилсом вела в профиль участника: у редакционного
   * поста профиля нет, и ссылки там не будет.
   */
  author: { id: string; name: string } | null;
  /**
   * Кто написал пояснение под цитатой. Отдельно от автора поста: пост
   * приносит один человек, а трактовку может написать другой — и спрашивать
   * за неё надо с него. `null` — пояснение собрала модель, подписывать нечем.
   */
  explanationAuthor: { id: string; name: string } | null;
  /** Пояснение спрятано по жалобам: наружу ушла одна цитата. */
  explanationHidden: boolean;
  /** Читатель уже пожаловался на это пояснение — кнопка показывает это. */
  explanationReported: boolean;
  /**
   * Глава Библиотеки, из которой выделен стих: по ней открывается комментарий.
   * Заполнена только у рилсов, собранных в читалке, — только там мы знаем
   * главу дословно. Пусто — кнопки «Комментарий» у слайда нет: вести в
   * поиск по названию значит обещать комментарий и не показать его.
   */
  library: { bookSlug: string; chapterSlug: string } | null;
