 ...
 * и на карточке планировщика.
 *
 * Псевдоним `TaskStatusMark`, а не свой набор: «в ленте Тестирование, а в
 * планировщике На доработку» (VED-320) случилось именно потому, что состояние
 * задачи считали в двух местах. Теперь понятие одно и живёт в `task-status.ts`;
 * имя остаётся, чтобы установленная сборка приложения продолжала собираться.
 */
export type NotificationMark = TaskStatusMark;
