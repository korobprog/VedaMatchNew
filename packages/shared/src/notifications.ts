...
export type NotificationEvent =
  ...
  | {
      ...
      actorName: string;
      /**
       * Колонка, в которой карточка лежит сейчас (VED-272). Факт, а не
       * формулировка: значок из неё собирает подписчик. Едет в событии,
       * потому что дочитать её из таблиц «Работы» он не вправе.
       */
      columnName: string;
    }
  | {
       ...
       */
      commentCount: number;
      /** Колонка карточки на момент комментария (VED-272), см. выше. */
      columnName: string;
    }
  | {
      /**
...
/**
 * Значок состояния рядом с уведомлением (VED-272).
 *
 * Одна и та же задача возвращается в ленту после каждой смены статуса, и без
 * пометки её приходится открывать заново, только чтобы понять, что именно
 * изменилось. Коды, а не русские слова: подписи, значки и цвета живут на
 * клиенте — как и все прочие формулировки портала.
 *
 * Список закрытый и короткий намеренно. Это четыре самых частых состояния
 * доски «Работа», а не справочник её колонок: колонку человек называет как
 * хочет, а значок обязан читаться одинаково у всех.
 */
...
export interface NotificationItemDto {
  ...
  readAt: string | null;
  /**
   * Состояние, о котором это уведомление (VED-272); `null` — состояния нет
   * или оно не из четвёрки известных. Проставляется в момент доставки и
   * больше не меняется: уведомление — это новость на свою дату, а не окно в
   * карточку. Текущий статус человек увидит, открыв её.
   */
