 ...
 *
 * Слова — те же, что человек видит в приложении и на сайте: «Музыка»
 * (сервис назывался «Медиатека», заказчик вернул прежнее имя — VED-426;
 * само название правит администратор в каталоге сервисов, см.
 * `config/services.ts`), «Общение» для чата, «Люди» для справочника.
 */
...
const MARK_VIEWS: Record<NotificationMark, MarkView> = {
  ...
  // «Чужое» (VED-320): задачу составил и ведёт другой. Акценты палитры все
  // заняты, поэтому рамка приглушённая, `text2`: не моё — не тревожит.
  foreign: { label: 'Чужое', border: 'text2' },
};
