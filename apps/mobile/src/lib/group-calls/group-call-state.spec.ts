...
describe('вход в комнату', () => {
  ...
  it('восстановление после перезапуска поднимает соединения заново', () => {
...
describe('состав комнаты меняется по ходу', () => {
  it('вошедший появляется в составе', () => {
    ...
    expect(state.call?.participants.map((p) => p.user.id)).toEqual(['a', 'me', 'b']);
  });

  it('вышедший уносит с собой состояние своего соединения и подпись «говорит»', () => {
    let state = joined(['a', 'me', 'b']);
    state = reduceGroupCall(state, { type: 'peer-state', userId: 'a', state: 'connected' });
    state = reduceGroupCall(state, { type: 'peer-state', userId: 'b', state: 'connecting' });
  ...
  it('события чужой комнаты игнорируются', () => {
    ...
    const after = reduceGroupCall(state, {
      type: 'stream',
      event: { type: 'group-call.updated', call: room(['x', 'y'], { id: 'room-2' }) },
      selfId: 'me',
    });
    expect(after).toBe(state);
  });

  it('сигналинг состояние экрана не трогает', () => {
    const state = joined(['a', 'me']);
    const after = reduceGroupCall(state, {
      type: 'stream',
      event: {
...
describe('микрофон и выход', () => {
  ...
  it('сервер может поправить состояние микрофона', () => {
    const state = reduceGroupCall(joined(['a', 'me']), { type: 'muted', muted: true });
...
describe('состояние отдельного соединения', () => {
  ...
  it('одинаковый список говорящих не создаёт нового состояния', () => {
    ...
    expect(reduceGroupCall(state, { type: 'speaking', userIds: ['a'] })).toBe(state);
  });
});
