...
describe('камера в фоне', () => {
  it('свёрнутое приложение камеру гасит', () => {
    expect(videoDimmedByBackground('background', false)).toBe(true);
  });

  it('картинка в картинке камеру не гасит — нас в ней видно', () => {
    expect(videoDimmedByBackground('background', true)).toBe(false);
  });

  it('шторка уведомлений камерой не моргает', () => {
    expect(videoDimmedByBackground('inactive', false)).toBe(false);
  });

  it('на переднем плане камера работает', () => {
    expect(videoDimmedByBackground('active', false)).toBe(false);
  });
});
...
describe('уходит ли наша картинка', () => {
  ...
  it('в фоне не уходит — батарея и нагрев', () => {
    expect(shouldSendGroupVideo({ ...base, appState: 'background' })).toBe(false);
  });

  it('в фоне с картинкой в картинке уходит', () => {
    expect(
      shouldSendGroupVideo({ ...base, appState: 'background', pipActive: true }),
    ).toBe(true);
  });
});

describe('плитки комнаты', () => {
  const room = call([participant('a', true), participant('b', true), participant('c')]);
  ...
  it('чужая картинка ждёт и согласия сервера, и реального потока', () => {
    // Сервер уже сказал «камера включена», но поток ещё не доехал —
    // показываем аватар, иначе на его месте будет чёрный прямоугольник.
    const tiles = videoTiles({
      call: room,
      selfId: 'a',
      sendingVideo: true,
  ...
  it('поток без согласия сервера картинкой тоже не считается', () => {
    const tiles = videoTiles({
      call: room,
      selfId: 'a',
      sendingVideo: true,
  ...
  it('свернувший приложение сосед показывается аватаром, а не замёрзшим кадром', () => {
  ...
  it('молчание про камеру трактуется как «снимает»', () => {
    // Сигнал мог не дойти (обрыв потока, перезапуск ICE) — гасить из-за
    // этого живую картинку нельзя.
    const tiles = videoTiles({
      call: room,
      selfId: 'a',
      sendingVideo: true,
...
describe('кнопка «камера»', () => {
  ...
  it('четвёртому отказ с понятным текстом', () => {
    ...
    expect(state.blockedReason).toBe('В групповом видео могут участвовать трое');
  });

  it('своя включённая камера не считается занятым чужим местом', () => {
    // Иначе третий не смог бы выключить и снова включить свою камеру.
    const room = call([
      participant('a', true),
      participant('b', true),
      participant('c', true),
  ...
  it('потолок берётся из ответа сервера, а не из константы сборки', () => {
    // Сборка приложения живёт дольше правила на сервере: если он скажет
    // «две камеры», кнопка обязана погаснуть на второй.
    const room = call([participant('a', true), participant('b', true), participant('c')], {
