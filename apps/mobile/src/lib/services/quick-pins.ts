 ...
 *
 * Пять — ровно столько чипов шириной 70 помещается в ряд на экране 412 dp
 * (Galaxy A51, на котором проверяется приложение) без прокрутки: 5×70 +
 * 4 промежутка по 6 + поля 12 с двух сторон = 398. Шестой уже уезжает за
 * край, и панель превращается во второй каталог, который надо листать, —
 * а каталог уже есть, на вкладке «Сервисы». Нижнее меню тоже из пяти
 * пунктов: верх и низ читаются одной сеткой.
 */
