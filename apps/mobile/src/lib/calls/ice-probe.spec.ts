import { buildProbePlan, formatSummary, judgeStep, parseCandidate } from './ice-probe';

describe('parseCandidate', () => {
  it('разбирает host, srflx и relay из строки SDP', () => {
    expect(parseCandidate('candidate:1 1 udp 2122260223 192.168.1.5 50000 typ host generation 0')).toEqual({
      type: 'host',
      protocol: 'udp',
    });
    expect(
      parseCandidate('a=candidate:2 1 udp 1686052607 85.10.1.1 50001 typ srflx raddr 192.168.1.5 rport 50000'),
    ).toEqual({ type: 'srflx', protocol: 'udp' });
    expect(
      parseCandidate('candidate:3 1 udp 41885439 45.150.9.229 49200 typ relay raddr 85.10.1.1 rport 50001'),
...
describe('buildProbePlan', () => {
  ...
  it('по одному шагу на транспорт, TURN разнесён по отдельным соединениям', () => {
    ...
    expect(plan.map((s) => s.transport)).toEqual(['stun', 'relay-udp', 'relay-tcp', 'relay-tls']);
    const tls = plan.find((s) => s.transport === 'relay-tls')!;
    expect(tls.servers).toEqual([
      { urls: ['turns:turn.example.org:5349?transport=tcp'], username: '1:u', credential: 'c' },
    ]);
  ...
  it('без TURN в плане только STUN', () => {
    expect(buildProbePlan([{ urls: ['stun:stun.l.google.com:19302'] }]).map((s) => s.transport)).toEqual(['stun']);
  });
});

describe('judgeStep', () => {
  const relayStep = buildProbePlan([{ urls: ['turn:h:3478?transport=udp'], username: 'u', credential: 'c' }])[0];
