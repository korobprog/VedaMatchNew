...
import { createBackgroundCallAction, declineCallInBackground as declineInstance, hangupCallInBackground as hangupInstance } from './background-call-action';
...
describe('createBackgroundCallAction — decline', () => {
  it('нет токена вовсе — не отправляет запрос, отдаёт false', async () => {
    const fetchImpl = jest.fn();
    const authority = fakeAuthority({ getAccessToken: async () => null });
    const act = createBackgroundCallAction({ fetchImpl, tokenAuthority: authority });
    await expect(act('call-1', 'decline')).resolves.toBe(false);
    expect(fetchImpl).not.toHaveBeenCalled();
  });

  it('access живой — один POST с Bearer на /decline без тела, успех', async () => {
    ...
    const act = createBackgroundCallAction({ fetchImpl, tokenAuthority: fakeAuthority() });
    await expect(act('call-1', 'decline')).resolves.toBe(true);
...
describe('createBackgroundCallAction — end (VED-222, п.1, feedback-001 блокирующий п.2)', () => {
  it('POST на /end с телом {reason}, по умолчанию "hangup"', async () => {
    const fetchImpl = jest.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      expect(String(input)).toBe('https://api.example/chat/calls/call-9/end');
      expect(init?.method).toBe('POST');
    ...
    const act = createBackgroundCallAction({ fetchImpl, tokenAuthority: fakeAuthority() });
    await expect(act('call-9', 'end')).resolves.toBe(true);
  });

  it('явный reason передаётся в теле как есть', async () => {
    const fetchImpl = jest.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
      expect(init?.body).toBe(JSON.stringify({ reason: 'network' }));
      return jsonResponse(200);
    });
    const act = createBackgroundCallAction({ fetchImpl, tokenAuthority: fakeAuthority() });
    await expect(act('call-9', 'end', 'network')).resolves.toBe(true);
  });

  it('401 → refresh → повтор тем же протоколом, что у decline', async () => {
    let calls = 0;
    const fetchImpl = jest.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
      calls += 1;
      const auth = (init?.headers as Record<string, string>).Authorization;
  ...
  it('нет токена — false, не отправляет запрос', async () => {
    const fetchImpl = jest.fn();
    const authority = fakeAuthority({ getAccessToken: async () => null });
    const act = createBackgroundCallAction({ fetchImpl, tokenAuthority: authority });
    await expect(act('call-9', 'end')).resolves.toBe(false);
    expect(fetchImpl).not.toHaveBeenCalled();
  });
});

describe('готовые инстансы — declineCallInBackground/hangupCallInBackground', () => {
  it('обе функции экспортированы и вызываемы (реальный singleton tokenAuthority — только форма, не сеть)', () => {
    expect(typeof declineInstance).toBe('function');
    expect(typeof hangupInstance).toBe('function');
  });
});
