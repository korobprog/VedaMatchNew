import { decideConnectingTimeout, type ConnectingTimeoutInput } from './call-connect-timeout';

function input(over: Partial<ConnectingTimeoutInput> = {}): ConnectingTimeoutInput {
  return {
    phase: 'connecting',
...
describe('decideConnectingTimeout', () => {
  ...
  it('звонок сменился, пока таймер ждал — ignore', () => {
    expect(decideConnectingTimeout(input({ currentCallId: 'call-2' }))).toBe('ignore');
  });

  it('звонка вовсе нет (сброшено в idle) — ignore', () => {
    expect(decideConnectingTimeout(input({ phase: 'idle', currentCallId: null }))).toBe(
      'ignore',
    );
  });
});
