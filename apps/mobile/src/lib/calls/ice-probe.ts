...
/**
 * Чистая часть зонда ICE — перенос без переписывания из
 * `apps/web/src/components/chat/calls/ice-probe.ts` (этап 0 плана
 * звонков, VED-218). Разбор кандидатов и план проверок живут здесь и
 * покрыты тестом; сам `RTCPeerConnection` (react-native-webrtc) — в
 * `ice-probe-runner.ts`, он не запускается в jest-expo/jsdom.
 */
...
/**
 * Один шаг на транспорт, а не одно соединение на всё: движок собирает
 * кандидаты параллельно и не говорит, через какой из URL пришёл relay.
 * Разнеся URL по отдельным соединениям, получаем честный ответ по каждому.
 */
...
export function judgeStep(step: ProbeStep, candidates: ParsedCandidate[]): boolean {
 ...
 */
export function formatSummary(results: StepResult[], loopback: StepOutcome): string {
  const mark = (o: StepOutcome) => (o === 'ok' ? 'да' : o === 'fail' ? 'нет' : '—');
  const cell = (t: ProbeTransport) => mark(results.find((r) => r.transport === t)?.outcome ?? 'pending');
  return [cell('stun'), cell('relay-udp'), cell('relay-tcp'), cell('relay-tls'), mark(loopback)].join(' | ');
}
