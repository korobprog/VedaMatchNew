...
describe('reduceVoiceRecorder', () => {
  ...
  /**
   * Регресс живой проверки сборки 5004 (Samsung A51): `voice-recorder-
   * control.tsx` раньше слал сюда `{ type: 'failed' }` при провале самого
   * СТАРТА записи (см. `voice-recorder-control.tsx: start()`) — но `failed`
   * действует только из `uploading` (тест выше), и вызов с базой `idle`
   * молча возвращал состояние БЕЗ ИЗМЕНЕНИЙ: UI оставался в `idle`, даже
   * если нативный `MediaRecorder` уже успел реально начать писать. Отдельное
   * действие `startFailed` — специально для провала старта, из `idle`.
   */
