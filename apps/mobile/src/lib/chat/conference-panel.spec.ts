...
describe('conferencePanelView — вход закрыт', () => {
  ...
  it('нерабочую ссылку не показывает и отправлять не предлагает', () => {
...
describe('conferenceCallCta', () => {
  it('идущий разговор зовут «войти», не начатый — «начать»', () => {
    expect(conferenceCallCta({ callLive: true })).toBe('Войти в разговор');
    expect(conferenceCallCta({ callLive: false })).toBe('Начать разговор');
  });
});

describe('conferenceActionNote', () => {
  it('говорит, что случилось, а не «успешно»', () => {
    expect(conferenceActionNote('shared')).toBe('Ссылка отправлена');
    ...
    for (const action of ['shared', 'revoked', 'rotated'] as const)
