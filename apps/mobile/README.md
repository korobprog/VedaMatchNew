...
- **Оболочка до JS.** `public/index.html` рисует фирменный знак и спиннер
  ...
  рендере.
- **Деление бандла по маршрутам.** `app.config.ts`:
  `['expo-router', { asyncRoutes: { web: true, default: 'development' } }]` —
  Android не трогает. Тяжёлые экраны (`call/[id]`, `chat/[id]`,
  `people/[id]`, `communities/[id]`, `account`, `calls-probe`, вкладки)
  грузятся отдельными чанками по переходу. `scripts/patch-web-preloads.mjs`
  (часть `export:web`) добавляет `<link rel="preload" as="script">` на чанки,
  нужные для первого экрана в 100% случаев (оба layout'а маршрутизатора,
  `login`, `auth`) — без этого маршрутизатор узнаёт о них только в рантайме и
  на медленной сети платит лишним оборотом сети на каждый файл; не
  `modulepreload` — чанки Metro грузятся обычным `<script>`, не нативным
  `import()`, и `modulepreload` для них не переиспользуется (см. комментарий
  в файле). Логика выбора чанков — `scripts/web-preload-chunks.mjs` (тест
  рядом, `test:web-preload-chunks`).
- **Известный компромисс.** У корневого layout'а два файла
  (`_layout.tsx`/`_layout.web.tsx`) — `expo-router` требует безымянный
  fallback рядом с платформенным, поэтому `_layout.tsx` со всеми шестью
  начертаниями `@expo-google-fonts` всё равно попадает в веб-сборку отдельным
  мёртвым чанком (комментарий в файле). `patch-web-preloads.mjs` не греет его
  предзагрузкой, но сам маршрутизатор один раз запрашивает его по сети —
  это и есть весь недостающий запас до полного паритета с базовым замером по
  «кнопкам входа».
