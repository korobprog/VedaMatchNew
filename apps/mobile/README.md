...
### Боевая сборка и сборка разработчика

| `APP_BUILD_KIND` | Пакет | Имя | Схема ссылок | Кто так собирает |
|---|---|---|---|---|
| `release` | `com.vedamatch.app` | VedaMatch | `vedamatch://` | воркфлоу Mobile APK, `Dockerfile.web` |
| не задана (или `dev`) | `com.vedamatch.app.dev` | VedaMatch Dev | `vedamatch-dev://` | всё остальное: `expo run:android`, ручная сборка APK |

Сборка разработчика ставится **рядом** с боевой, а не поверх. Раньше ручная
сборка шла под боевым пакетом: на Samsung A51 стояла такая с versionCode
5025, сборки с сайта (от 1000) поверх неё не вставали, самообновление её не
видело — приложение пришлось удалять вместе с данными. Умолчание выбрано в
сторону разработчика намеренно. Если забыть переменную в CI, получится APK с
заметным именем «VedaMatch Dev», и он просто не встанет поверх боевого. Если бы
умолчание было боевым, забытая переменная на машине разработчика снова дала бы
боевой пакет с чужим номером. Опечатка в значении роняет сборку. Код —
`src/config/build-kind.ts`, проверки — `build-kind.spec.ts` и `app-config.spec.ts`
(в том числе что воркфлоу и `Dockerfile.web` ставят `release`).

Что отличается у сборки разработчика:

- **Схема `vedamatch-dev://`.** Иначе две установленные сборки делили бы
  возврат из браузера после входа: Android спросил бы, чем открыть, или
  отдал бы код сборке без PKCE-верификатора. API принимает оба адреса
  возврата (`APP_REDIRECT_URIS` в `apps/api/src/modules/auth/app-login.ts`).
  Ссылки на конференции `vedamatch://j/…` с сайта открывает только боевая.
- **Без FCM, пока в Firebase нет её пакета.** `google-services.json` выписан
  на `com.vedamatch.app`, и gradle-плагин Google Services падает на пакете,
  которого в файле нет. Поэтому файл подключается сборке разработчика, только
  если в нём есть и `com.vedamatch.app.dev`. Когда пакет заведут в проекте
  `vedamathai` и скачают обновлённый файл, пуши появятся сами. RuStore-пуша в
  приложении нативно нет, его это не касается.
- Иконка та же, отличается подпись «VedaMatch Dev».

Проверить, что получится: `npx expo config --type public` (без переменной и с
`APP_BUILD_KIND=release`) — поля `name`, `scheme`, `android.package`.

Остальные переменные сборки:
...
| `APP_DOWNLOAD_BASE_URL` | Запасной адрес раздачи самообновления (канал `site`), тот же, что `S3_PUBLIC_URL` портала: `<APP_DOWNLOAD_BASE_URL>/mobile/android/<контур>-<канал>/latest.json`. Первым приложение спрашивает API портала — см. «Откуда приложение берёт манифест» ниже. Путь в адресе разрешён (`https://host/bucket`), хвостовой слэш срезается; окончание `/test` ведёт и запрос к API в тестовую папку. В CI берётся из секрета `S3_PUBLIC_URL`, только для `channel=site` |
...
Сначала — API портала: `GET <apiOrigin>/notifications/app-release/<контур>-<канал>/latest.json`
(публичный, без входа; `?track=test` — тестовая папка). Не ответил, 404 или
503 — зашитый в сборку `APP_DOWNLOAD_BASE_URL`. Порядок —
`src/lib/self-update/manifest-sources.ts`, таймауты: API 8 с, хранилище 15 с.

Зачем: 24.09 хранилище сменило домен (s3.firstvds.ru → firsts3.ru → прокси
media.vedamatch.ru), и все вышедшие сборки, знавшие только зашитый адрес,
остались без обновлений навсегда. Домен API не меняется. Сборкам до этого
изменения это не поможет — их код об API не знает.

Сервер читает манифест из хранилища по своему `APP_DOWNLOAD_BASE_URL`
(прямой адрес, в проде `https://firsts3.ru/<бакет>`), с таймаутом 5 с и
кэшем в памяти на минуту; сбой хранилища — 503. Публичный адрес
(`S3_PUBLIC_URL`, прокси media) сервер не читает никогда — 24.09 чтение
собственного публичного домена уронило сайт
(`apps/api/src/modules/notifications/app-release.ts`, `manifestStorageBase`).
...
`vedamatch://auth` (у сборки разработчика — `vedamatch-dev://auth`),
приложение меняет его на пару токенов через
...
Сессия восстанавливается из токенов в SecureStore; сеть нужна только чтобы
перечитать профиль (`GET /users/me`). Правила — `src/lib/startup/startup-decision.ts`:

- токена нет — экран входа сразу, сети не ждём;
- токен есть, `expo-network` говорит «сети нет» — вкладки сразу, над ними
  плашка «Нет соединения», профиль догружается, когда сеть вернётся;
- токен есть, сеть есть или неизвестно — ждём профиль не дольше 8 с
  (у `fetch` на Android своего таймаута нет), потом тоже вкладки;
- если «загрузка» всё равно тянется дольше 12 с — экран «Нет соединения» с
  «Повторить» и автоповтором при появлении сети, а не пустой экран.

Из аккаунта выводит только явный отказ сервера на обновлении токена
(401/403), не отсутствие сети.
...
### Входы воркфлоу Mobile APK

| Вход | По умолчанию | Что делает |
|---|---|---|
| `contour` / `channel` | `ru` / `site` | Контур и канал сборки (таблица «Сборки» выше) |
| `publish` | выкл. | Выложить APK и `latest.json` в S3 (только `channel=site`) |
| `test_folder` | выкл. | Выкладывать в `test/mobile/android/…` и собирать клиент с `APP_DOWNLOAD_BASE_URL=<S3_PUBLIC_URL>/test` — пользователи сайта эту папку не видят |
| `version_code_base` | `1000` | `versionCode = база + номер запуска`. Только десятичное число без ведущего нуля |
| `release_notes` | пусто | «Что нового» — поле `notes` в `latest.json`. Сервер пишет его в пост официального канала VedaMatch о выходе версии; перенос строки — `\n`, не длиннее 1000 символов. Пусто — в посте «Исправления и улучшения.» |

**Осторожно с `version_code_base`.** Самообновление сравнивает только
`versionCode`: телефон с тестовой сборкой 2012 не увидит боевых обновлений,
пока боевой номер не перевалит за 2012, а установка с сайта поверх упрётся в
понижение версии (помогает только удаление приложения). Поэтому
`publish=true` с базой, отличной от `1000`, без `test_folder=true` воркфлоу
отклоняет с ошибкой — иначе весь сайт уехал бы на большой номер без пути
назад. Проверка — `scripts/release-version.mjs`, тест
`pnpm --filter @vedamatch/mobile test:release-version`.
Тестовый и боевой прогоны с одной ветки живут в разных группах
`concurrency` и друг друга не отменяют.
...
API раз в минуту читает тот же `latest.json`
(`apps/api/src/modules/notifications/app-release-worker.service.ts`). Номер
больше всех известных — новая версия: пост «Вышла версия N» в официальном
канале VedaMatch и пуш «Доступна новая версия» телефонам со сборкой `*-site`,
чей `appVersionCode` (приложение присылает его при регистрации токена) меньше
вышедшего. Один пуш на выпуск, с 9 до 21 по поясу человека, только при
включённой категории «Новости портала»; магазинная сборка пуш не получает.
Самую первую версию, которую сервер застал при выкатке, и откат на прежний
APK он не объявляет. Публикация в `test/` сервер не трогает.

## Firebase

`google-services.json` в репозиторий не кладётся. Проект Firebase —
`vedamathai`, пакет — `com.vedamatch.app`. Сборке разработчика
(`com.vedamatch.app.dev`) файл подключается, только если в нём есть и её
пакет, см. «Боевая сборка и сборка разработчика».
