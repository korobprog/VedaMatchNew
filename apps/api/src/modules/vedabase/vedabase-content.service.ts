...
export class VedabaseContentService {
  ...
  async getBook(slug: string) {
    return this.notFound(() => this.repository.getActiveBook(slug));
  }
  async getChapter(bookSlug: string, chapterSlug: string) {
    return this.notFound(() =>
      this.repository.getActiveChapterRecord(bookSlug, chapterSlug),
    );
  }
  async getSearchIndex(slug: string) {
    return this.notFound(() => this.repository.getOfflineSearchIndex(slug));
  }
  search(query: string, limit?: string) {
    ...
    if (!Number.isInteger(parsed) || parsed < 1)
      throw new BadRequestException('Invalid search limit');
