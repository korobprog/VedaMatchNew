...
export class VedabaseContentRepository {
  ...
  async listActiveBooks(): Promise<VedabaseLibraryManifest> {
    ...
    return {
      ...
      books: books.flatMap((book) =>
        book.activeVersion ? [this.toManifest(book)] : [],
      ),
    };
  }
  ...
  async getActiveChapter(
    bookSlug: string,
    chapterSlug: string,
  ): Promise<VedabaseChapter> {
    return (await this.getActiveChapterRecord(bookSlug, chapterSlug)).chapter;
  }

  async getActiveChapterRecord(
    bookSlug: string,
    chapterSlug: string,
  ): Promise<{ chapter: VedabaseChapter; sha256: string }> {
    ...
    return {
      chapter: row.payload as unknown as VedabaseChapter,
      sha256: row.sha256,
    };
  }

  async getOfflineSearchIndex(
    bookSlug: string,
  ): Promise<VedabaseSearchDocument[]> {
    const book = await this.findActiveBook(bookSlug);
  ...
  async activateVersion(versionId: string): Promise<void> {
    await this.prisma.$transaction(
      async (transaction) => {
        const version = await transaction.vedabaseBookVersion.findUnique({
          where: { id: versionId },
        });
        if (!version || version.status !== 'validated')
          throw new Error('Vedabase version is not validated');
        const book = await transaction.vedabaseBook.findUnique({
          where: { id: version.bookId },
        });
        if (!book) throw new Error('Vedabase book not found');
        await transaction.vedabaseBookVersion.updateMany({
          where: { bookId: book.id, status: 'active' },
          data: { status: 'validated' },
        });
        await transaction.vedabaseBookVersion.update({
          where: { id: version.id },
          data: { status: 'active' },
        });
        await transaction.vedabaseBook.update({
          where: { id: book.id },
  ...
  private async findActiveBook(slug: string): Promise<ActiveBook> {
    ...
    if (!book?.activeVersion || book.activeVersion.bookId !== book.id)
      throw new Error('Vedabase book not found');
    return book;
  }
  ...
  private toManifest(book: ActiveBook): VedabaseBookManifest {
    ...
    if (!Number.isSafeInteger(size))
      throw new Error('Vedabase book size exceeds JSON safe integer');
    ...
    return {
      ...
      sizeBytes: size,
      coverPath: book.cover ? 'cover' : null,
      ...
      files: [
        ...chapters.map((chapter) => ({
          path: `chapters/${chapter.slug}.json`,
          bytes: chapter.bytes,
        ...
        })),
        {
          ...
          bytes: version.searchIndexBytes,
          sha256: version.searchIndexSha256,
          contentType: 'application/json; charset=utf-8',
        },
      ],
    };
