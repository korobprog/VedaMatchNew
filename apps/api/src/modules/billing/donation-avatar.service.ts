 ...
 * импортировать `UsersService`, а фото получателей пожертвований на странице
 * «Поддержать» нужно такое же, как в портальном профиле. Устройство
 * скопировано с `PeopleAvatarService` — по контракту хелперы дублируются.
 */
@Injectable()
export class DonationAvatarService {
