 ...
 *
 * Своя роль — потолок: администратор не производит ни второго
 * администратора, ни владельца. Владелец назначает любую роль, в том числе
 * «владелец» (VED-422): заказчик — второй человек проекта, и ему нужна
 * подпись «Владелец», не отнимая владения у первого. Совладелец — это
 * владелец по правам, кроме того, что держится за основным владельцем
 * (`WorkSpace.ownerId`): удалить среду и передать владение может только
 * основной, и понизить или исключить основного нельзя никому.
 */
export function canAssignRole(
  ...
  return RANK[target] < RANK[actor] || actor === 'owner';
}

/**
 * Почему участника нельзя понизить или исключить; `null` — можно.
 *
 * - Основного владельца — никому: на нём держится среда (удалится его
 *   аккаунт — удалится и она), он уходит только передачей владения.
 * - Совладельца — только владелец: иначе администратор снимал бы тех, кто
 *   выше его.
 */
export function memberChangeProblem(params: {
  actor: WorkMemberRole;
  target: WorkMemberRole;
  targetIsPrimaryOwner: boolean;
}): string | null {
  if (params.targetIsPrimaryOwner) {
    return 'Основного владельца нельзя понизить или исключить: сначала передайте владение';
  }
  if (params.target === 'owner' && params.actor !== 'owner') {
    return 'Совладельца может понизить или исключить только владелец';
  }
  return null;
}
