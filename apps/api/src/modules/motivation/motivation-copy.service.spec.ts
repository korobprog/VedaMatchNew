...
function validCopy() {
  return {
    ...
    explanation:
      'Эта цитата напоминает о бескорыстной заботе и внимании к другим.',
    translations: {
      ru: {
        ...
        label: null,
        title: 'Служение',
        explanation: 'Эта цитата напоминает о бескорыстной заботе.',
        storyText: 'Любовь в служении',
      },
      en: {
        quoteText: 'Service is love.',
        translationKind: 'vedamatch',
        label: 'Перевод VedaMatch',
        title: 'Service',
        explanation: 'The quote points to selfless care.',
        storyText: 'Love through service',
      },
      hi: {
        quoteText: 'सेवा प्रेम है।',
        translationKind: 'vedamatch',
        label: 'Перевод VedaMatch',
        title: 'सेवा',
        explanation: 'यह उद्धरण निःस्वार्थ देखभाल की याद दिलाता है।',
        storyText: 'सेवा में प्रेम',
      },
    },
  };
}
...
describe('MotivationCopyService', () => {
  const quote = {
    ...
    sourceUrl: null,
    contextExcerpt: `Контекст: ${originalText}`,
    verified: true,
  };

  function setup(copy = validCopy()) {
    const transaction = {
      motivationQuoteTranslation: {
        deleteMany: jest.fn(),
        createMany: jest.fn(),
      },
      ...
      motivationPost: {
        create: jest
          .fn()
          .mockResolvedValue({ id: 'post-1', reviewStatus: 'text_review' }),
      },
    };
    const prisma = {
  ...
  it('classifies, restores the original and persists a text-review candidate', async () => {
    ...
    expect(transaction.motivationQuoteProfile.createMany).toHaveBeenCalledWith({
      data: [{ quoteId: 'quote-1', profileType: 'devotee' }],
    });
    expect(
      transaction.motivationQuoteTranslation.createMany,
    ...
    expect(transaction.motivationPost.create).toHaveBeenCalledWith({
  ...
  it('returns an existing post for the quote without calling AI', async () => {
    ...
    prisma.motivationPost = {
      findUnique: jest.fn().mockResolvedValue(existing),
    };

    await expect(service.prepareCandidate('quote-1')).resolves.toEqual(
      existing,
    );
  ...
  it.each([
    ...
    [
      ...
      'one or two non-empty paragraphs',
    ],
    [
      {
        ...validCopy(),
        translations: {
          ...validCopy().translations,
          en: { ...validCopy().translations.en, label: null },
        },
      },
      'translation label',
    ],
    [
      {
        ...
        translations: {
          ...validCopy().translations,
          en: { ...validCopy().translations.en, translationKind: 'official' },
        },
      },
      'translation kind',
    ],
    [
      {
        ...
        translations: {
          ...validCopy().translations,
          en: { ...validCopy().translations.en, explanation: '' },
        },
      },
      'en explanation',
    ],
    [
      {
        ...
        translations: {
          ...validCopy().translations,
          hi: {
            ...validCopy().translations.hi,
            explanation:
              'पहला पर्याप्त अनुच्छेद।\n\nदूसरा पर्याप्त अनुच्छेद।\n\nतीसरा पर्याप्त अनुच्छेद।',
          },
        },
      },
      'hi explanation',
  ...
  it('rejects missing or unverified quotes before calling AI', async () => {
    ...
    await expect(service.prepareCandidate('missing')).rejects.toBeInstanceOf(
      NotFoundException,
    );
    expect(generation.generateVerifiedQuoteCopy).not.toHaveBeenCalled();

    prisma.motivationQuote.findUnique.mockResolvedValue({
      ...quote,
      verified: false,
    });
    await expect(service.prepareCandidate('quote-1')).rejects.toBeInstanceOf(
      BadGatewayException,
    );
  });
});
