...
import {
  ...
  type VerifiedQuote,
} from './quote-verification.service';

const DISCOVERY_QUERIES = [
  'жизнь or мудрость or цель or служение or сострадание or мужество or покой or истина',
];

export type IngestableQuoteCandidate =
  | VerifiedQuote
  | NonNullable<ReturnType<QuoteDiscoveryService['verifyWebCandidate']>>;
...
export class QuoteDiscoveryService {
  ...
  async discoverDaily(date: Date, count = 8): Promise<MotivationQuote[]> {
    ...
    const candidates = new Map<
      string,
      VerifiedQuote | ReturnType<typeof this.verifyWebCandidate>
    >();
    ...
    const internalExisting = await this.findExistingHashes([
      ...candidates.keys(),
    ]);
    const freshInternalCount = [...candidates.keys()].filter(
      (hash) => !internalExisting.has(hash),
    ).length;
    if (freshInternalCount < missingCount) {
      const webResults = await this.web.search(
        DISCOVERY_QUERIES.join(' '),
        (missingCount - freshInternalCount) * 4,
      );
    ...
    const selected = [...candidates.entries()]
      ...
      .slice(0, missingCount) as Array<
      VerifiedQuote | NonNullable<ReturnType<typeof this.verifyWebCandidate>>
    >;
    if (selected.length !== missingCount)
      throw new Error('insufficient_verified_quotes');
    ...
    return this.prisma.$transaction(
      async (transaction) => {
        ...
        if (selected.length < stillMissing)
          throw new Error('insufficient_verified_quotes');

        await transaction.motivationQuote.createMany({
          data: selected
            .slice(0, stillMissing)
            .map((candidate) => ({ ...candidate, discoveryDate })),
          skipDuplicates: true,
        });
  ...
  async ingestCandidate(
    candidate: IngestableQuoteCandidate,
  ): Promise<MotivationQuote | null> {
    const existing = await this.prisma.motivationQuote.findUnique({
      where: { normalizedHash: candidate.normalizedHash },
    });
    if (existing) return null;
    return this.prisma.motivationQuote.create({
      data: { ...candidate, discoveryDate: null },
    });
  }

  verifyWebCandidate(candidate: WebQuoteCandidate) {
    if (!candidate.verified || !candidate.sourceUrl || !candidate.author)
      return null;
    if (!candidate.contextExcerpt.includes(candidate.originalText)) return null;
    return {
      originalText: candidate.originalText,
      normalizedHash:
        candidate.normalizedHash || quoteFingerprint(candidate.originalText),
