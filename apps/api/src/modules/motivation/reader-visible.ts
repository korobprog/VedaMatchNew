import { MotivationPostOrigin, MotivationPostStatus } from '@prisma/client';

/**
 * Что во «Вдохновении» вообще может увидеть читатель.
 *
 * Опубликованное — минус рилсы участников без проверенного источника: такой
 * рилс живёт во вкладке «Мои» и по прямой ссылке, а в общую ленту и в папки
 * не идёт. Читателю он недоступен ни одним путём.
 *
 * Афоризм администратора сервиса из-под этого правила выведен (VED-9): он и
 * есть тот, кто отвечает за ленту, и прятать его публикацию не от кого.
 * Раньше администратор добавлял афоризм своим текстом, видел его только во
 * вкладке «Мои» и решал, что добавление не работает вовсе.
 *
 * Готовая картинка участника (`captionInImage`, VED-97) тоже выведена:
 * цитата напечатана на самой картинке, сверять её не с чем, и владелец решил
 * пускать такие картинки в общую ленту сразу.
 *
 * Правило вынесено сюда, потому что по нему считают в двух местах — счётчик
 * «N вдохновений в сервисе» над лентой и числа у папок в оглавлении. Пока
 * счётчик считал просто всё опубликованное, он обещал больше, чем есть: над
 * лентой стояло одно число, сумма по папкам давала другое, и расхождение
 * читалось как пропажа публикаций.
 *
 * Подбор под путь читателя сюда не входит намеренно: направления у каждого
 * свои, и число, меняющееся от галочки в настройках, читалось бы так же
 * плохо. Это правило отвечает на вопрос «сколько всего есть для читателя», а
 * не «сколько покажут мне».
 */
export const READER_VISIBLE_POSTS = {
  status: MotivationPostStatus.published,
  NOT: {
    origin: MotivationPostOrigin.user,
    sourceVerified: false,
    authorIsAdmin: false,
    captionInImage: false,
  },
};
