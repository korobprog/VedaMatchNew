...
export class MotivationGenerationService {
    ...
    this.s3 =
      region && accessKeyId && secretAccessKey
        ? new S3Client({
            ...
            forcePathStyle: Boolean(config.get('S3_ENDPOINT')),
  ...
  private validateExtractedQuotes(value: unknown): Array<{
    ...
    if (!value || typeof value !== 'object')
      throw new BadGatewayException(
        'Text provider returned invalid quotes payload',
      );
    const quotes = (value as { quotes?: unknown }).quotes;
    if (!Array.isArray(quotes))
      throw new BadGatewayException('Text provider returned no quotes array');
    return quotes
      .flatMap((item) => {
        ...
        return [
          {
            ...
            originalLanguage:
              ...
              record.originalLanguage.trim()
                ? record.originalLanguage.trim()
                : 'en',
            contextExcerpt:
              typeof record.contextExcerpt === 'string' &&
              record.contextExcerpt.trim()
                ? record.contextExcerpt.trim()
                : originalText,
          },
        ];
  ...
  async generateImage(prompt: string): Promise<Buffer> {
    ...
    const timeoutMs = Number(
      this.config.get<string>('MOTIVATION_IMAGE_TIMEOUT_MS') || 180_000,
    );
    ...
    if (
      ...
      throw new BadGatewayException('Image provider returned no valid PNG');
    return bytes;
  }
  ...
  async generateApprovedImage(input: {
    ...
    if (
      ...
      throw new BadRequestException(
        'Image generation requires approved text and a stored image prompt',
      );
    }
  ...
  private validateCopy(value: unknown) {
    if (!value || typeof value !== 'object')
      throw new BadGatewayException('Text provider returned invalid copy');
    ...
    return ['ru', 'en', 'hi'].map((language) => {
      const item = translations[language] as
        Record<string, unknown> | undefined;
      if (
        !item ||
        typeof item.title !== 'string' ||
        typeof item.text !== 'string' ||
        typeof item.storyText !== 'string'
      )
        throw new BadGatewayException(`Text provider omitted ${language}`);
      return {
        language,
