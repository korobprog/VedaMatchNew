  ...
  private reelDto(
    ...
    return {
      ...
      post: {
        ...
        origin: 'user',
        author: null,
        // Своя же карточка в кабинете: подписывать пояснение автору перед ним
        // самим незачем — он и так знает, чьё оно. Жаловаться на своё
        // пояснение тоже некому.
        explanationAuthor: null,
        explanationHidden: false,
        explanationReported: false,
        // Кабинет автора показывает карточку, а не слайд ленты: комментарий
        // из Библиотеки открывают там, где читают, — в ленте.
        library: null,
        isOwn: true,
      },
    };
