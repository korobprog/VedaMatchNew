...
export class MotivationAuthorSearchService {
  ...
  async searchByWatchId(watchId: string): Promise<number> {
    const watch = await this.prisma.motivationAuthorWatch.findUnique({
      where: { id: watchId },
    });
    ...
    if (ingestedCount < MIN_INTERNAL_RESULTS_BEFORE_WEB) {
      ...
      for (const candidate of webResults) {
        if (!candidate.author.toLocaleLowerCase().includes(normalizedAuthor))
          continue;
  ...
  private async ingest(
    candidate: Parameters<QuoteDiscoveryService['ingestCandidate']>[0],
  ): Promise<boolean> {
