...
/*
 * Рендер через sharp: первый вызов в воркере jest собирает кэш шрифтов
 * fontconfig, и на свежем раннере CI, где параллельно гоняются остальные
 * наборы, это дольше пяти секунд по умолчанию — тесты с отрисовкой падали по
 * таймауту, не дойдя до проверки (main 7bc1f55, e0cda47). Локально весь файл
 * идёт за пару секунд; запас нужен именно на холодный старт.
 */
