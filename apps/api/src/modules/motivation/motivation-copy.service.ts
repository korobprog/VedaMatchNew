...
export class MotivationCopyService {
  ...
  async prepareCandidate(quoteId: string, category?: string) {
    ...
    const quote = await this.prisma.motivationQuote.findUnique({
      where: { id: quoteId },
    });
    if (!quote) throw new NotFoundException('Verified quote not found');
    if (!quote.verified)
      throw new BadGatewayException('Quote source is not verified');
    ...
    const profileTypes = [
      ...new Set(copy.profileTypes),
    ] as MotivationProfileType[];
    const profileType = profileTypes[0];
    const contentDate =
      quote.discoveryDate ?? new Date(new Date().toISOString().slice(0, 10));
    const audienceTrack =
        ...
        data: languages.map((language) => ({
          ...
          quoteText:
            language === quote.originalLanguage
              ? quote.originalText
              : copy.translations[language].quoteText,
          translationKind: copy.translations[language]
            .translationKind as MotivationTranslationKind,
          label: copy.translations[language].label,
        })),
      });
      await transaction.motivationQuoteProfile.deleteMany({
        where: { quoteId },
      });
      await transaction.motivationQuoteProfile.createMany({
        data: profileTypes.map((assignedProfile) => ({
          quoteId,
  ...
  private validateCopy(
    ...
    if (
      !copy ||
      typeof copy !== 'object' ||
      !Array.isArray(copy.profileTypes) ||
      typeof copy.explanation !== 'string' ||
      !copy.translations ||
      typeof copy.translations !== 'object'
    ) {
      throw new BadGatewayException(
        'Text provider returned invalid verified quote copy',
      );
    }
    if (copy.originalText !== originalText)
      throw new BadGatewayException('Text provider returned modified original');
    if (
      !copy.profileTypes.length ||
      copy.profileTypes.some((profile) => !allowedProfiles.has(profile))
    )
      throw new BadGatewayException('Text provider returned unknown profile');
    ...
    for (const language of languages) {
      ...
      if (
        ...
        typeof translation.quoteText !== 'string' ||
        typeof translation.title !== 'string' ||
        typeof translation.explanation !== 'string' ||
        typeof translation.storyText !== 'string'
      )
        throw new BadGatewayException(
          `Text provider omitted ${language} translation`,
        );
      ...
      if (language === originalLanguage) {
        if (translation.quoteText !== originalText)
          throw new BadGatewayException(
            'Text provider returned modified original translation',
          );
        if (translation.translationKind !== 'official')
          throw new BadGatewayException(
            'Original translation kind must be official',
          );
        ...
        throw new BadGatewayException(
          'AI translation label must be Перевод VedaMatch',
        );
      }
    }
  }
  ...
  private validateExplanation(explanation: string, field: string) {
    ...
    if (
      ...
      throw new BadGatewayException(
        `${field} must contain one or two non-empty paragraphs longer than 20 characters`,
      );
    }
  }
}
