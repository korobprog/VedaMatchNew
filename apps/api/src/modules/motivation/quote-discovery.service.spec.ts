...
describe('QuoteDiscoveryService', () => {
  it('returns the same verified UTC daily batch without searching or creating more quotes', async () => {
    ...
    const first = await service.discoverDaily(
      new Date('2026-07-13T18:45:00.000Z'),
      8,
    );
    const second = await service.discoverDaily(
    ...
    expect(prisma.motivationQuote.findMany).toHaveBeenCalledWith(
  ...
  it('fills only the missing part of an existing UTC daily batch', async () => {
    ...
    expect(transaction.motivationQuote.createMany).toHaveBeenCalledWith({
      data: expect.arrayContaining([
        expect.objectContaining({ discoveryDate }),
      ]),
      skipDuplicates: true,
    });
    expect(
      transaction.motivationQuote.createMany.mock.calls[0][0].data,
    ).toHaveLength(5);
  });
  ...
  it('uses internal search before web, drops duplicates and unverified results, and saves eight', async () => {
        ...
        return [
          {
            ...
            work: 'Wikiquote',
            locator: '',
            sourceUrl: 'https://en.wikiquote.org/wiki/Duplicate',
            contextExcerpt: 'Internal exact quote 1',
            verified: true,
          },
          {
            ...
            work: 'Wikiquote',
            locator: '',
            sourceUrl: 'https://en.wikiquote.org/wiki/Unknown',
            contextExcerpt: 'Different text',
            verified: false,
          },
    ...
    tx.motivationQuote.findMany = jest
      .fn()
  ...
  it('does not count an already persisted hash toward the daily eight', async () => {
    ...
    const prisma = {
      motivationQuote: {
        findMany: jest.fn(async ({ where }) =>
          where.discoveryDate ? [] : [{ normalizedHash: existingHash }],
        ),
      },
      $transaction: jest.fn(async (callback) => callback(transaction)),
    };
    ...
    expect(saved.some((quote) => quote.normalizedHash === existingHash)).toBe(
      false,
    );
  });
