import { buildBlogPosterArgs, parseFfmpegInfo } from './blog-video';
...
describe('buildBlogPosterArgs', () => {
  it('seeks before the input, takes one frame and writes png', () => {
    const args = buildBlogPosterArgs({
      videoPath: '/t/in.mp4',
      posterPath: '/t/p.png',
    });
  ...
  it('keeps the info log level that prints the input', () => {
    const args = buildBlogPosterArgs({ videoPath: 'a', posterPath: 'b' });
