...
/** Ширина обложки — как у фотографий ленты: она стоит в той же карусели. */
...
/**
 * Обёртка над ffmpeg (VED-116): снять обложку и замерить ролик.
 *
 ...
 * всё решаемое вынесено в `blog-video.ts`.
 */
@Injectable()
export class BlogVideoService {
  ...
  async inspect(
    video: Buffer,
    extension: string,
  ): Promise<{ info: BlogVideoInfo; poster: Buffer } | null> {
    const dir = await mkdtemp(join(tmpdir(), 'vm-blog-'));
    ...
    try {
      ...
      const stderr = await this.run(
        ffmpegPath(),
        buildBlogPosterArgs({ videoPath, posterPath }),
      );
      ...
      this.logger.warn(
        `Ролик поста не разобран: ${error instanceof Error ? error.message : String(error)}`,
      );
      return null;
    } finally {
