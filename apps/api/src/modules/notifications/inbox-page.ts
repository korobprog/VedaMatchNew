 ...
 *
 * Постраничность не навязывается: клиент, который её не просил, получает
 * ленту целиком — см. `LEGACY_INBOX_LIMIT` и `isPaginationRequested()`.
 */
...
/**
 * Сколько отдаётся клиенту, который о постраничности не знает (VED-267).
 *
 * Установленное приложение просит ленту без параметров и второй раз не
 * приходит: отдать ему двадцать записей — значит показать человеку обрезанную
 * ленту, а остальное спрятать без следа. Поэтому без курсора и без `limit`
 * лента отдаётся целиком, как отдавалась раньше.
 *
 * Но не совсем без предела: запрос без ограничения — отдельная беда, не
 * связанная с приложением. Тысяча взята с большим запасом к сегодняшним
 * лентам (у самого нагруженного человека на проде 190 записей) и при этом
 * не даёт одному аккаунту с десятью тысячами уведомлений положить сервер.
 * Если предел сработал, ответ говорит об этом полем `truncated`, а не молчит.
 */
export const LEGACY_INBOX_LIMIT = 1000;

/**
 * Просил ли клиент постраничность. Курсор или размер порции в запросе —
 * значит просил; пустой запрос — это старый клиент, ему лента целиком.
 *
 * Пустая строка считается отсутствием: `?cursor=&limit=` получается сам собой
 * у клиента, который подставляет пустые значения, и означать «дай двадцать»
 * она не должна.
 */
export function isPaginationRequested(
  cursor: unknown,
  limit: unknown,
): boolean {
  const given = (raw: unknown) => {
    if (raw === undefined || raw === null) return false;
    if (typeof raw === 'string') return raw.trim().length > 0;
    return true;
  };
  return given(cursor) || given(limit);
}
