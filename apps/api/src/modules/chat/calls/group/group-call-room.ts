...
export interface RoomParticipant {
  ...
  muted: boolean;
  /**
   * Камера включена — то есть занято одно из мест под видео
   * (`group-call-video.ts`). Правила состава про это поле намеренно ничего
   * не знают: потолок участников и потолок камер считаются раздельно, и
   * смешивать их нельзя — человек с выключенной камерой остаётся
   * полноправным участником разговора.
   */
  video: boolean;
}
