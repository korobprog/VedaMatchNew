/**
 * Разбор ролика статуса (VED-129): кадр-обложка, длительность и размер кадра.
 * Копия `blog/blog-video.ts` (VED-116) — модуль чата не импортирует блог.
 *
 * Аргументы ffmpeg и разбор его вывода — отдельным модулем с тестами, а
 * обёртка со `spawn` (`chat-status-video.service.ts`) — без них: ошибка в строке
 ...
 * любая; в webp обложку пережимает `sharp`, как и все картинки переписки.
 *
 ...
 */
export function buildStatusPosterArgs(input: {
  videoPath: string;
  posterPath: string;
}): string[] {
  return [
...
export interface StatusVideoInfo {
 ...
 * высоту — иначе вертикальный ролик ляжет горизонтальной рамкой.
 */
export function parseFfmpegInfo(stderr: string): StatusVideoInfo {
