...
import type { ContactsDetails } from './people.service';
import { PeopleService } from './people.service';
import { isVerifiedDevotee } from './people-verification';
 ...
 *
 * Отдельный сервис от `PeopleService` намеренно: там модель видимости
 * карточки, здесь — доступ к способам связи. Проверка видимости не дублируется,
 * а переиспользуется вызовом `PeopleService.getCard`.
 */
@Injectable()
export class PeopleRequestsService {
  constructor(
    private readonly prisma: PrismaService,
    private readonly contacts: PeopleService,
    private readonly moderation: ModerationService,
    private readonly users: PeopleAvatarService,
