-- Файлы портала отдаются через свой домен media.vedamatch.ru
-- (portal/media-proxy): сертификат firsts3.ru выдан от корня GlobalSign
-- Root R46, которого нет в хранилище доверия старых Android, и картинки,
-- аудио и манифест обновлений там не открывались. Бакет и пути те же,
-- меняется только домен — тем же обходом колонок, что и в
-- 20260926190000_s3_firsts3_domain. Повторный прогон ничего не найдёт.
DO $$
DECLARE
  col record;
  n bigint;
  total bigint := 0;
  old_host constant text := 'https://firsts3.ru/';
  new_host constant text := 'https://media.vedamatch.ru/';
BEGIN
  FOR col IN
  ...
  LOOP
    ...
    IF n > 0 THEN
      RAISE NOTICE 's3 media domain: %.% — % rows', col.table_name, col.column_name, n;
      total := total + n;
    END IF;
  END LOOP;
  RAISE NOTICE 's3 media domain: total % rows', total;
END $$;
