...
const services = [
  ...
  },
  {
    ...
    url: '/travel',
    // Открыт: ночлег, заявки, касса хостела, страница по QR, переписка с
    // хозяином и отзывы готовы. Статус — поведение, сид перезаписывает его
    // при каждом старте api, поэтому переключать его надо здесь, а не в
    // админке.
    status: 'active',
    category: 'lifestyle',
    public: true,
