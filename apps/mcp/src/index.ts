...
server.registerTool(
  'work_create_task',
  {
    ...
    description:
      'Создать карточку на доске — в указанную колонку, а без неё в первую. Исполнителя и срок можно указать сразу или проставить потом через work_update_task.',
    inputSchema: {
      boardId: z.string().describe('Доска, на которой заводится задача'),
      // Без этого поля zod молча выбрасывал колонку из запроса, и задача
      // всегда уезжала в первую колонку (VED-73).
      columnId: z
        .string()
        .optional()
        .describe(
          'Колонка из work_get_board, например «Тестерование»; без неё — первая колонка доски',
        ),
