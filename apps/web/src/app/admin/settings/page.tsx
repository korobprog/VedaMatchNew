...
import { getAdminBillingMode, getAdminDonationSettings, getProfile } from "@/lib/api";
...
export default async function AdminSettingsPage() {
  ...
  const [billingMode, donation] = await Promise.all([
    getAdminBillingMode(),
  ...
  return (
    <div className="relative min-h-dvh bg-bg-0">
      ...
      <main className="mx-auto max-w-2xl px-4 py-8 pb-24">
        ...
        <div className="mt-6">
          <AdminDonationForm initial={donation ?? { enabled: false, text: "", requisites: [] }} />
        </div>
      </main>
    </div>
  );
