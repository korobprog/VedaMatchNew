...
const publicPrefixes = [
  ...
  "/services",
  "/vaishnava",
  // Экран приглашения в рабочую среду: ссылку пересылают в мессенджер, и
  // получатель может ещё не иметь аккаунта. Он должен увидеть, КУДА его зовут,
  // до входа; сам вход спросится на кнопке «принять».
  "/work/join/",
  // Акт за период коммерческой доски (VED-461): ссылку ведущий отправляет
  // клиенту, у которого аккаунта может не быть вовсе.
  "/work/act/",
  // Страница объекта «Ночлега» по QR со стойки хостела: гость сканирует код
  // телефоном, аккаунта у него обычно нет. Заявку он оставит и без входа.
  "/travel/s/",
  // Короткая ссылка на быструю конференцию (VED-360). Её пересылают в
  // мессенджер человеку, которого в портале может ещё не быть: он обязан
  // увидеть, кто зовёт и сколько мест, ДО входа. Сам вход спросится на
  // кнопке, и `?returnTo=` вернёт его ровно сюда — в том числе после
  // первого входа, который в портале и есть регистрация.
  "/j/",
];
