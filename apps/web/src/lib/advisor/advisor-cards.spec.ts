...
describe("greetFirst", () => {
  ...
  it("гасит заглавную после имени", () => {
    ...
    expect(first.text).toBe(
      "Марина, без города Объявления и справочник людей не покажут, что происходит рядом с вами",
    );
  });
  ...
  it("не гасит имя собственное в начале карточки", () => {
    // «Сита, астрология ничего не рассчитает» — так нельзя.
    const [first] = greetFirst(
      buildAdvisorCards({ ...calm, astroPercent: 0 }),
      "Сита",
    );
    expect(first.text).toBe(
      "Сита, Астрология ничего не рассчитает, пока нет точного времени и места рождения",
    );
  });
