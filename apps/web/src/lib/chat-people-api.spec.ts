...
describe("Contacts API requests", () => {
  it("asks contacts/search with the built query", async () => {
    ...
    expect(fetchMock.mock.calls[0]?.[0]).toBe(
      "http://localhost:4000/chat/people/search?q=%D0%BF%D0%BE%D0%B2%D0%B0%D1%80&tagIds=a&tagIds=b",
    );
  });
  ...
  it("escapes the id in the contacts/users path", async () => {
    ...
    expect(fetchMock.mock.calls[0]?.[0]).toBe(
      "http://localhost:4000/chat/people/users/a%20b%2Fc",
    );
  });
  ...
  it("puts the built body to contacts/profile with cookies", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:4000/chat/people/profile",
      {
        credentials: "include",
        method: "PUT",
        headers: { "Content-Type": "application/json" },
...
describe("requests and disclosures endpoints", () => {
  ...
  it("reads both lists from contacts/requests", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:4000/chat/people/requests",
  ...
  it("posts the answer to the request it belongs to", async () => {
    ...
    expect(fetchMock.mock.calls[0]?.[0]).toBe(
      "http://localhost:4000/chat/people/requests/r%201/respond",
    );
  ...
  it("cancels an outgoing request by id", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:4000/chat/people/requests/r1",
  ...
  it("reads and revokes disclosures", async () => {
    ...
    expect(fetchMock.mock.calls[0]?.[0]).toBe(
      "http://localhost:4000/chat/people/disclosures",
    );
    expect(fetchMock.mock.calls[1]).toEqual([
      "http://localhost:4000/chat/people/disclosures/d1",
      { credentials: "include", method: "DELETE" },
    ]);
  });
});
