...
export function moveTaskLocally(
  ...
  //
  // Ярлык состояния переезжает вместе с карточкой по той же причине, но цена
  // ошибки здесь выше галочки: доску после переноса никто не перечитывает, и
  // ярлык от прежней колонки врал бы до перезагрузки страницы — ровно та
  // жалоба, которую чинит VED-320, только теперь внутри самой доски. Код
  // состояния берём готовым у колонки: разбор её названия один на портал и
  // живёт на сервере (`work-task-status.ts`).
  const moved: WorkTaskCardDto = {
    ...task,
    columnId: targetColumnId,
    statusMark: target.statusMark,
