...
import { PeopleSearchView } from "./people-search-view";
...
function stubApi(search: ContactsSearchResponse) {
  const fetchMock = vi.fn<typeof fetch>().mockImplementation(async (input) => {
    ...
    const body = url.includes("/chat/people/tags") ? { items: tags } : search;
...
describe("PeopleSearchView", () => {
  ...
  it("drops the page number when the filters change", async () => {
    ...
    await waitFor(() => expect(push).toHaveBeenCalledWith("/chat/people"));
  });
