...
import { PeopleMapPanel } from "./people-map-panel";
import { PeopleSearchCard } from "./people-search-card";
import { PeopleSearchFiltersPanel } from "./people-search-filters";
import {
...
/**
 * `query` — строка запроса страницы, её отдаёт сервер. Раньше компонент брал
 * её `useSearchParams`, но тогда весь экран уходил в клиентский рендер: React
 * оставлял границу Suspense отложенной, и при прямом заходе по ссылке
 * справочник не появлялся вовсе.
 */
export function PeopleSearchView({ query = "" }: { query?: string }) {
  ...
  function pushFilters(next: ContactsSearchFilters) {
    ...
    router.push(search ? `/chat/people?${search}` : "/chat/people");
  }
