 ...
 * non-blocking п.1, усиливающий блокирующий п.1).
 *
 ...
 * совпадал с порядком генерации на клиенте: более старый сигнал мог
 * обогнать более новый (например, офер повторной попытки — легитимный офер
 * ICE-рестарта той же стороны, отправленный чуть позже).
 *
...
export class SignalSendQueue {
  ...
  enqueue<T>(task: () => Promise<T>): Promise<T> {
    ...
    // Хвост очереди не должен обрываться ошибкой одной задачи — иначе все
    // следующие задачи в цепочке зависли бы навсегда.
