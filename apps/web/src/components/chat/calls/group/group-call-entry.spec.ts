...
function room(count: number, over: Partial<ChatGroupCallDto> = {}): ChatGroupCallDto {
  return {
    ...
    status: "live",
    hostId: "u0",
    startedBy: { id: "u0", name: "u0", avatarUrl: null, lastSeenAt: null },
...
describe("подпись кнопки", () => {
  ...
  it("третий в комнате на четверых кнопку не гасит", () => {
    expect(groupCallButtonLabel(room(3), "idle").disabled).toBe(false);
  });

  it("закончившийся звонок войти не предлагает", () => {
    expect(groupCallButtonLabel(room(2, { status: "ended" }), "idle")).toEqual({
      text: "Звонок уже закончился",
      disabled: true,
    });
  });
