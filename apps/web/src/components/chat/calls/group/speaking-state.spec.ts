...
describe("кто говорит", () => {
  ...
  it("включил микрофон обратно — подпись снова доступна", () => {
    let state = nextSpeakingState(EMPTY_SPEAKING_STATE, { a: 0.4 }, 1000, new Set(["a"]));
    expect(speakingIds(state)).toEqual([]);
    state = nextSpeakingState(state, { a: 0.4 }, 1400);
    expect(speakingIds(state)).toEqual(["a"]);
  });

  it("исчезнувший из замера считается молчащим", () => {
    let state = nextSpeakingState(
      EMPTY_SPEAKING_STATE,
      { a: 0.2, b: 0.2 },
      1000,
    );
