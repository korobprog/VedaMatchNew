...
/**
 * Кто с кем соединяется в mesh'е — решение на стороне браузера.
 *
 * Правило то же, что у сервера (`apps/api/.../group/group-call-room.ts`) и
 * у приложения (`apps/mobile/src/lib/group-calls/group-call-peers.ts`), и
 * продублировано намеренно: клиент обязан уметь пересчитать план
 * соединений из состава комнаты, который он и так получает событием, не
 * спрашивая сервер отдельным запросом. Расходиться этим трём местам
 * нельзя, поэтому каждое покрыто одним и тем же тестом на симметрию пары.
 *
 * Суть: позже вошедший делает offer всем, кто уже был. Без такого правила
 * обе стороны пары выставляют offer одновременно (glare), `webrtc-signal-
 * guard.ts` честно отбрасывает один, и соединение не поднимается вовсе —
 * жёсткие роли caller/callee звонка один на один здесь не годятся:
...
function orderedIds(
  participants: readonly ChatGroupCallParticipantDto[],
): string[] {
  return [...participants]
    .sort((a, b) => {
 ...
 *
 * `current` — с кем соединение уже есть (ключи карты `GroupPeerLink` в
...
export function planPeers(
  ...
  if (selfIndex < 0)
    // Нас в комнате нет: держать соединения не с кем, все прежние — на
    // закрытие.
...
/** Сколько соединений держит вкладка при таком составе. */
