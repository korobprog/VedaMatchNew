...
describe("attributionLine", () => {
  ...
  it("не повторяет название произведения, если оно уже есть в начале главы", () => {
    // Иногда генерация кладёт название книги ещё раз в номер стиха
    // («Бхагавад-гита 6.1» вместо «6.1») — тогда оно дублируется в строке.
    expect(
      attributionLine(
        post({
    ...
    ).toBe("А. Ч. Бхактиведанта Свами Прабхупада · Бхагавад-гита как она есть · 6.1");
  });

  it("сравнивает без учёта регистра и не трогает главу без совпадения", () => {
    expect(
      attributionLine(
        post({ attributionWork: "бхагавад-гита", attributionLocator: "БХАГАВАД-ГИТА 2.47" }),
      ),
    ).toBe("бхагавад-гита · 2.47");
    expect(
      attributionLine(post({ attributionWork: "Бхагавад-гита", attributionLocator: "6.1" })),
    ).toBe("Бхагавад-гита · 6.1");
  });
});
