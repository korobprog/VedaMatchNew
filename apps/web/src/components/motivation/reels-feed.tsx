...
function Tabs({
  ...
  return (
    // VED-252, круг 2: цифры ниже — не оценка по символам, а живой замер в
    // браузере на 360×780 (Chrome DevTools, круг 2, коммит 778cca75; шрифт
    // реальный, не подставной). Кнопки ←/меню занимают x 8–48 и x 312–352
    // (`left-2/right-2` + `size-10`), свободный коридор между ними —
    // 48–312 = 264px. `inset-x-12` (48px с каждой стороны) берёт ряд ровно
    // по этому коридору, без наложения и без лишнего запаса воздуха.
    // Измеренная ширина пяти пунктов: «Лента» 39px, «Открытки» 62px, значок
    // фильтра 40px (полный `w-10` — хит-зона, а не оценка «на глаз»),
    // «Избранное» 71px, «Мои» 27px — сумма 239px. При `gap-x-1` (4px × 4
    // промежутка = 16px) итог 255px против 264px доступных — запас 9px,
    // а не 1–3px, как было в первой версии на оценке по символам.
    // `top-2` — тот же отступ, что у ←/меню (`ReelsChrome`, `left-2 top-2`),
    // ряд `items-center` по высоте самого высокого пункта (значок, `h-10`) —
    // так центр ряда совпадает с центром кнопок по краям, а не сидит ниже.
    // `flex-wrap` — страховка на экранах у́же 360px, не расчёт на неё здесь.
    <nav
      aria-label="Вкладки ленты"
      className="absolute inset-x-12 top-2 z-20 flex flex-wrap items-center justify-center gap-x-1 gap-y-1 sm:inset-x-0 sm:gap-x-3"
    >
