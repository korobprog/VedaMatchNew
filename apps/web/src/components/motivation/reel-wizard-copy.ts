...
export function stageItems(stage: MotivationReelStage): StageItem[] {
  ...
  const hints: Record<MotivationReelStage, [string, string, string, string]> = {
    ...
    // Про уведомление сказано прямо на шаге, а не только мелкой строкой внизу:
    // человек смотрит на «Картинку» и решает, ждать ему или уходить.
    generating: [
      "Принята",
      "Пройдена",
      "Рисуем иллюстрацию — можно заниматься своими делами, мы пришлём уведомление",
      "Публикация в ленту",
    ],
