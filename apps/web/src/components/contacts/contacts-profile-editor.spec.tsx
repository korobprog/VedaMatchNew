...
import { ContactsProfileEditor } from "./contacts-profile-editor";
...
function stubApi(profile: ContactsProfileDto | null, items = tags) {
  const fetchMock = vi.fn<typeof fetch>().mockImplementation(async (input) => {
    ...
    const body = url.endsWith("/contacts/tags") ? { items } : { profile };
...
describe("ContactsProfileEditor", () => {
  it("shows a loading state and then the form", async () => {
    stubApi(baseProfile);
    render(<ContactsProfileEditor />);
  ...
  it("sends the edited card and reports success", async () => {
    ...
    expect(put?.[0]).toBe("http://localhost:4000/contacts/profile");
  ...
  it("keeps the backend error text on a failed save", async () => {
    ...
    const fetchMock = vi.fn<typeof fetch>().mockImplementation(async (input, init) => {
      ...
      const body = String(input).endsWith("/contacts/tags")
        ? { items: tags }
        : { profile: baseProfile };
      return new Response(JSON.stringify(body), {
        headers: { "Content-Type": "application/json" },
