 ...
 * (ERR_CONNECTION_ABORTED), а в установленном портале новая вкладка вообще
 * открывается пустым белым окном — показать её негде. Схему `tg://` телефон
 * отдаёт установленному приложению, не загружая никакой страницы, —
 * блокировать и рисовать нечего.
 *
 * Разбор адреса вынесен сюда отдельной функцией: обёртка вокруг него в
 * карточке материала не тестируется, а таблица форм ссылок — да.
 *
 * Такой же разбор есть у ленты афоризмов и у экрана «Поделиться». Копия здесь
 * намеренная: контракт сервисного модуля запрещает тянуть хелпер из чужой
 * папки, общее правило дублируется.
 */
