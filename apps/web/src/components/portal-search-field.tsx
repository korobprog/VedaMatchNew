...
/**
 * Поле поиска по порталу (VED-75): над выдачей на `/search`. С главной поле
 * убрано (VED-238) — на ту же страницу ведёт плитка «Поиск» в панели горячих
 * кнопок.
 *
...
export function PortalSearchField({
  ...
  autoFocus?: boolean;
  /**
   * Поле по ширине надписи, а не на всю строку (VED-89). Осталось от
   * главной, где поле приглашало искать и не должно было спорить с блоками
   * ниже; сейчас нигде не включено. Над выдачей поле широкое всегда: там
   * правят длинный запрос.
   */
  compact?: boolean;
  className?: string;
}) {
