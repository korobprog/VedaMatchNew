...
/**
 * Иконки интересов для чипсов в карточке знакомства. Интересы можно вписывать
 * свободно, поэтому карта неполна по определению — для своего варианта
 * подставляется «искра».
 */
const unionInterestEmoji: Record<string, string> = {
  философия: "📜",
  йога: "🧘",
  медитация: "🕉️",
  киртан: "🪘",
  "ведическая культура": "🏛️",
...
export function unionInterestIcon(interest: string): string {
  return unionInterestEmoji[interest.trim().toLowerCase()] ?? "✨";
}
