...
/**
 * Значит ли событие `pause`, что звук остановили (VED-132).
 *
 * Дослушанная до конца запись шлёт `pause` раньше `ended` — так устроен
 * `<audio>`. Плеер принимал его за «человек нажал паузу», гасил признак
 * «играет», и следующая запись, которую выбирал `ended`, вставала в полосу,
 * но не звучала: автозапуск ждёт как раз этого признака. Пауза в конце
 * записи — не остановка: решает, что дальше, обработчик `ended`.
 */
export function pauseStopsPlayback(audio: { ended: boolean }): boolean {
  return !audio.ended;
}
