...
export function SelfIdentificationForm({
  ...
  const [result, setResult] = useState<SelfIdentificationSubmitResult | null>(
    null,
  );
  ...
  const mentorPath = result?.mentorLinkPath ??
    (state?.activeMentorRequest
      ? `/mentor-verification/${state.activeMentorRequest.token}`
      : null);
  ...
  const currentStage = result?.spiritualStage ?? state?.spiritualStage;
  const currentStatus =
    result?.devoteeVerificationStatus ?? state?.devoteeVerificationStatus;
