...
Замеры 2026-09-09, зонд `/chat/calls/probe` (в скобках — мс до первого
нужного кандидата):

| Сеть | STUN | relay UDP | relay TCP | relay TLS | петля |
|---|---|---|---|---|---|
| Дом, провайдер в РФ, Windows | да (198) | да (360) | да (697) | да (668) | да |
| Телефон, LTE, РФ | да (823) | да (465) | да (711) | да (962) | да |
| Машина разработчика, TURN Allocate скриптом | — | да | да | да | — |

Вывод этапа 0: TURN в Амстердаме доступен из российских сетей по всем трём
транспортам, UDP не задушен, NAT провайдера пробрасывает релейный диапазон.
Второй coturn в московском контуре не нужен; отдельный IP под 443 — тоже.
Порог остаётся правилом на будущее: если relay TCP/TLS из РФ станет
проходить реже, чем в 9 случаях из 10, сначала TURN в Москве, потом код.
