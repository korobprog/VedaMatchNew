...
## CI: паритет веб → приложение

`.github/workflows/web-app-parity.yml` (VED-215) заводит карточку «ДОГНАТЬ»
на доске «Работа», когда мерж в `main` меняет сервис веба или его API —
подробности правила и исключений в
[docs/service-module-contract.md](docs/service-module-contract.md#паритет-с-мобильным-приложением-ved-215).
Воркфлоу нужен секрет репозитория:

| Секрет | Назначение |
|---|---|
| `VEDAMATCH_API_KEY` | Персональный ключ доступа с правом `work:write` (Профиль → Ключи доступа), от чьего имени создаются карточки и комментарии |

Без секрета шаг только выводит `::warning::` и ничего не создаёт — сборка не
падает. Добавляется в Settings → Secrets and variables → Actions.
